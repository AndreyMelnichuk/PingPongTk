...
def check_score():
    ...
    if PLAYER_1_SCORE == 5 or PLAYER_2_SCORE == 5:
        ...
        if PLAYER_1_SCORE > PLAYER_2_SCORE:
            ...
            los = PLAYER_2_SCORE
            winner_str = "Player BLUE"
        else:
            winner_str = "Player RED"
        ...
        l = Label(t,font=helv36, text=winner_str+" WIN!!!\n With score: "+str(winner)+" : "+str(los))
        l.pack(side="top", fill="both", expand=True, padx=100, pady=100)
        b = Button(t, text="Retry", command=restart, bg="white",fg="blue", font=helv36)
        b.pack()
        c = Button(t, text="Exit", command=exit,  bg="white",fg="blue", font=helv36)
        c.pack()

        # c.unbind("<KeyPress>", movement_handler)
        # c.unbind("<KeyRelease>")

def restart():
    global PLAYER_1_SCORE, PLAYER_2_SCORE, flag
    t.destroy()
    PLAYER_1_SCORE = -1
    PLAYER_2_SCORE = -1
    update_score("left")
...
# устанавливаем окно
root = Tk()
root.title("Чемпионат Мира по Пинг-Понгу 2017")

# область анимации
c = Canvas(root, width=WIDTH, height=HEIGHT, background="#003330")
c.pack()
...
BALL = c.create_oval(WIDTH / 2 - BALL_RADIUS / 2,
                     HEIGHT / 2 - BALL_RADIUS / 2,
                     WIDTH / 2 + BALL_RADIUS / 2,
                     HEIGHT / 2 + BALL_RADIUS / 2, fill="yellow")
